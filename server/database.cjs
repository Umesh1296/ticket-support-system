...
function initializeDatabase() {
  ...
  return {
    async getManagers(query = {}) { return Manager.find(query).lean() },
    async getEmployees(query = {}) { return Employee.find(query).lean() },
    async getOperators(query = {}) { return Operator.find(query).lean() },
    ...
    // Super Admin CRUD
    async getSuperAdmins() { return SuperAdmin.find().lean() },
    async findSuperAdminById(id) { return SuperAdmin.findOne({ id }).lean() },
    async findSuperAdminByEmail(email) { return SuperAdmin.findOne({ email: new RegExp(`^${email}$`, 'i') }).lean() },
    async insertSuperAdmin(sa) {
      const doc = new SuperAdmin(sa)
      await doc.save()
      return doc.toObject()
    },
    async updateSuperAdmin(id, updates) { return SuperAdmin.findOneAndUpdate({ id }, updates, { new: true }).lean() },
    async deleteSuperAdmin(id) { return SuperAdmin.findOneAndDelete({ id }).lean() },
  }
}
