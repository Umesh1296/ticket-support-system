...
module.exports = (store) => {
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      const allOperators = await store.getOperators({ manager_id: managerId })
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      if (!normalizedSkills.length) {
        return res.status(400).json({ success: false, error: 'Add at least one support skill' })
      }
      ...
      const fbUser = await createFirebaseAuthUser(email.toLowerCase(), resolvedPassword, name)
  ...
  router.put('/:id', async (req, res) => {
    try {
      ...
      if (!operator || operator.manager_id !== req.user.id) {
        return res.status(404).json({ success: false, error: 'Operator not found' })
      }
      const updates = {}
  ...
  router.post('/:id/reset-password', async (req, res) => {
    try {
      ...
      let fbUid = operator.firebase_uid
      if (!fbUid) {
        const fbUser = await createFirebaseAuthUser(operator.email, resolvedPassword, operator.name)
        fbUid = fbUser.uid
      } else {
        const admin = require('firebase-admin')
  ...
  router.delete('/:id', async (req, res) => {
    try {
      ...
      if (operator.firebase_uid) {
        await deleteFirebaseAuthUser(operator.firebase_uid)
      }
